...
The reusable `deploy.yml` workflow reads per-project configuration from repository variables and secrets (Settings > Secrets and variables > Actions). **Non-sensitive identifiers** are stored as Variables; values that identify the Workload Identity authentication target are stored as Secrets so GitHub redacts them from public logs.

Configure these before running CI/CD in a new GCP project:
