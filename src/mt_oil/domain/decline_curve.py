...
def modified_arps_decline(t: np.ndarray, qi: float, di: float, b: float, d_lim: float = 0.06) -> np.ndarray:
    """
...
def fit_best_decline(time_months: np.ndarray, production: np.ndarray, method: str = "auto") -> Dict:
    """
    ...
    if method in ["arps", "modified_arps", "auto"]:
        try:
            ...
            popt, _ = curve_fit(arps_decline, time_months, production, 
    ...
    if method in ["duong", "auto"]:
        try:
            ...
            popt, _ = curve_fit(duong_decline, time_months, production,
                                p0=[qi_guess, 1.0, 1.1],
